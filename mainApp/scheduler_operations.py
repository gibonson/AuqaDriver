...
def sched_start(sched, schedulerNameToRun=None):
    ...
    for eventScheduler in eventSchedulerLister:
        ...
        logger.debug(
            f"schedulerId = {schedulerName} - trigger = {trigger}, day = {day}, day_of_week = {day_of_week}, hour = {hour}, minute = {minute}, second = {second}"
        )
...
def add_job_to_scheduler(
    sched, job_type, scheduler_id, args, trigger, day, day_of_week, hour, minute, second
):
    try:
        if trigger == "interval":
            sched.add_job(
                id=scheduler_id,
                func=globals()[job_type],
                args=[args],
                trigger=trigger,
                hours=hour,
                minutes=minute,
